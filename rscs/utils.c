...
// EPC Gen2 specifies the CRC16 polynomial as x^16 + x^12 + x^5 + 1 using CRC-CCITT
...
//     uint16_t crc = utils_crc_ccitt(0xFFFFU, buffer, len);
//     fprintf(stdout, "[dbg] utils_crc_ccitt_genibus: crc = 0x%04X\n", crc);
//     fprintf(stdout, "[dbg] utils_crc_ccitt_genibus: len = %zu\n", len);
//     fprintf(stdout, "[dbg] utils_crc_ccitt_genibus: buffer = ");
...
/**
 * @note This function doesn't perform any sanity checks on the input parameters apart from parity check (it will decrement u8len if it's odd)
 */
void utils_buf_u8_to_u16_big_endian(uint16_t *dst, const uint8_t *src, size_t u8len) {
    if (u8len % 2 != 0) {
        u8len--;
        fprintf(stdout, "WARNING: utils_buf_u8_to_u16_big_endian: u8len is not even\n");
    }
