...
mbuffer_normalized_unit_t mbuffer_get_normalized_unit(mbuffer_t *mbuf, uint8_t antenna_id, uint32_t tag_index) {
  ...
  for (size_t i = 0; i < NUM_CHANNELS; i++) {
    ...
    normalized_unit.rssi[i] = (double)(unit.rssi[i] - RSSI_MIN_VALUE) / (RSSI_MAX_VALUE - RSSI_MIN_VALUE);
    // scale phase_angle from 0..4095 to 0.0..1.0
    normalized_unit.phase_angle[i] = (double)(unit.phase_angle[i]) / 4095.0;
    // scale doppler_frequency from -32768..32767 to 0.0..1.0
    normalized_unit.doppler_frequency[i] = (double)(unit.doppler_frequency[i] - RSSI_MIN_VALUE) / (double)(32767 - RSSI_MIN_VALUE);
  }
...
void mbuffer_flush(mbuffer_t *mbuf) {
  ...
  for (size_t i = 0; i < NUM_ANTENNAS * (NUM_REFERENCE_TAGS + NUM_TRACKED_ASSETS); i++) {
    for (size_t j = 0; j < NUM_CHANNELS; j++) {
      mbuf->data[i].rssi[j] = RSSI_MIN_VALUE; // Minimum int16_t value
      mbuf->data[i].phase_angle[j] = 0;
      mbuf->data[i].doppler_frequency[j] = 0;
