...
uhfman_err_t uhfman_get_select_param(uhfman_ctx_t* pCtx, uint8_t* pTarget_out, 
    ...
    switch (rv) {
        case YPDR200_X0B_ERR_SUCCESS:
            ...
            // }
            // LOG_I_CFIN("]");
            if (pTarget_out) *pTarget_out = respParam.hdr.target;
            if (pAction_out) *pAction_out = respParam.hdr.action;
            if (pMemBank_out) *pMemBank_out = respParam.hdr.memBank;
            if (pPtr_out) *pPtr_out = (respParam.hdr.ptr[0] << 24) | (respParam.hdr.ptr[1] << 16) | (respParam.hdr.ptr[2] << 8) | respParam.hdr.ptr[3];
            if (pMaskLen_out) *pMaskLen_out = respParam.hdr.maskLen;
            if (pTruncate_out) *pTruncate_out = respParam.hdr.truncate;
            if (ppMask_out) {
                uint8_t mask_nbytes = (respParam.hdr.maskLen + 7) >> 3;
                *ppMask_out = (uint8_t*)malloc(mask_nbytes);
                memcpy(*ppMask_out, respParam.pMask, mask_nbytes);
            }
            // Update cached parameters
            uhfman_config_select_params_update(pCtx, respParam.hdr.target, respParam.hdr.action, respParam.hdr.memBank, 
                                                (respParam.hdr.ptr[0] << 24) | (respParam.hdr.ptr[1] << 16) | (respParam.hdr.ptr[2] << 8) | respParam.hdr.ptr[3], 
                                                respParam.hdr.maskLen, respParam.hdr.truncate, respParam.pMask);
...
uhfman_err_t uhfman_get_query_params(uhfman_ctx_t* pCtx, uhfman_query_sel_t* pSel_out, uhfman_query_session_t* pSession_out, uhfman_query_target_t* pTarget_out, uint8_t* pQ_out) {
    ...
    switch (rv) {
        case YPDR200_X0D_ERR_SUCCESS:
            ...
            if (pSel_out) {
                switch (respParam.sel) {
                    case 0x00:
                        *pSel_out = UHFMAN_QUERY_SEL_ALL;
                        break;
                    case 0x01:
                        *pSel_out = UHFMAN_QUERY_SEL_ALL1;
                        break;
                    case 0x02:
                        *pSel_out = UHFMAN_QUERY_SEL_NOT_SL;
                        break;
                    case 0x03:
                        *pSel_out = UHFMAN_QUERY_SEL_SL;
                        break;
                    default:
                        LOG_W("Unsupported sel field value: 0x%02X", respParam.sel);
                        return UHFMAN_GET_QUERY_PARAMS_ERR_NOT_SUPPORTED;
                }
            }
            if (pSession_out) {
                switch (respParam.session) {
                    case 0x00:
                        *pSession_out = UHFMAN_QUERY_SESSION_S0;
                        break;
                    case 0x01:
                        *pSession_out = UHFMAN_QUERY_SESSION_S1;
                        break;
                    case 0x02:
                        *pSession_out = UHFMAN_QUERY_SESSION_S2;
                        break;
                    case 0x03:
                        *pSession_out = UHFMAN_QUERY_SESSION_S3;
                        break;
                    default:
                        LOG_W("Unsupported session field value: 0x%02X", respParam.session);
                        return UHFMAN_GET_QUERY_PARAMS_ERR_NOT_SUPPORTED;
                }
            }
            if (pTarget_out) {
                switch (respParam.target) {
                    case 0x00:
                        *pTarget_out = UHFMAN_QUERY_TARGET_A;
                        break;
                    case 0x01:
                        *pTarget_out = UHFMAN_QUERY_TARGET_B;
                        break;
                    default:
                        LOG_W("Unsupported target field value: 0x%02X", respParam.target);
                        return UHFMAN_GET_QUERY_PARAMS_ERR_NOT_SUPPORTED;
                }
            }
            if (pQ_out) {
            ...
            // Update cached parameters
            uhfman_config_query_params_update(pCtx, *pSel_out, *pSession_out, *pTarget_out, *pQ_out);
            // No need to free response parameters as there are no dynamically allocated members
...
uhfman_err_t uhfman_get_demod_params(uhfman_ctx_t* pCtx, int* pMixerG_out, int* pIfG_out, uint16_t* pThrd_out) {
    ...
    switch (rv) {
        case YPDR200_XF1_ERR_SUCCESS:
            ...
            *pMixerG_out = (int)(respParam.mixer_G);
            *pIfG_out = (int)(respParam.if_G);
            *pThrd_out = thrd;
...
static void __uhfman_tag_submit_read(uint8_t* epc, uint8_t rssi, void* pUserData) {
    ...
    uhfman_tag_t tagInfo = {
        .epc = {0}
    };
    memcpy(tagInfo.epc, epc, YPDR200_X22_NTF_PARAM_EPC_LENGTH);
    __uhfman_poll_handler(tagInfo, rssi, pUserData);
