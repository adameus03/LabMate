...
static uint8_t dtr = 0;
static uint8_t rts = 0;
//static uint8_t do_exit = 0;
static uint8_t recvbuf[1024];

static void ch340_write_handshake_byte( libusb_device_handle *devh ) {
...
static int ch340_set_baudrate( libusb_device_handle *devh, int baudRate ){
...
int ch340_init( libusb_device_handle *devh )
{
    ...
    ch340_set_baudrate(devh, DEFAULT_BAUD_RATE);
    ch340_write_handshake_byte(devh);
