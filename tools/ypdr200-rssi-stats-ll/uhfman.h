 ...
 * @param pTarget_out Address where the target value will be stored
 * @param pAction_out Address where the action value will be stored
 * @param pMemBank_out Address where the memory bank value will be stored
 * @param pPtr_out Address where the pointer value will be stored
 * @param pMaskLen_out Address where the mask length value will be stored
 * @param pTruncate_out Address where the truncate value will be stored
 * @param ppMask_out Address where the mask pointer will be stored (dynamically allocated, must be freed by caller)
 */
uhfman_err_t uhfman_get_select_param(uhfman_ctx_t* pCtx, uint8_t* pTarget_out, 
                                                        uint8_t* pAction_out, 
                                                        uint8_t* pMemBank_out, 
                                                        uint32_t* pPtr_out, 
                                                        uint8_t* pMaskLen_out, 
                                                        uint8_t* pTruncate_out, 
                                                        uint8_t** ppMask_out);
 ...
 * @param pSel_out Address where the select value will be stored
 * @param pSession_out Address where the session value will be stored
 * @param pTarget_out Address where the target value will be stored
 * @param pQ_out Address where the Q value will be stored
 */
uhfman_err_t uhfman_get_query_params(uhfman_ctx_t* pCtx, uhfman_query_sel_t* pSel_out, uhfman_query_session_t* pSession_out, uhfman_query_target_t* pTarget_out, uint8_t* pQ_out);
 ...
 * @param pChIndex_out Address where the channel index will be stored
 */
uhfman_err_t uhfman_get_working_channel(uhfman_ctx_t* pCtx, uint8_t* pChIndex_out);

#define UHFMAN_WORK_AREA_CHINA_9 0x01
#define UHFMAN_WORK_AREA_CHINA_8 0x04
#define UHFMAN_WORK_AREA_US 0x08
#define UHFMAN_WORK_AREA_EU 0x03
#define UHFMAN_WORK_AREA_KOREA 0x06
...
/**
 * @brief Get work area
 * @param pRegion_out Address where the region code will be stored
 */
uhfman_err_t uhfman_get_work_area(uhfman_ctx_t* pCtx, int* pRegion_out);
 ...
 * @param pTxPower_dBm_out Address where the transmit power in dBm will be stored
 */
uhfman_err_t uhfman_get_transmit_power(uhfman_ctx_t* pCtx, float* pTxPower_dBm_out);
...
#define UHFMAN_RX_DEMOD_MIXER_G_0DB 0x00
#define UHFMAN_RX_DEMOD_MIXER_G_3DB 0x01
#define UHFMAN_RX_DEMOD_MIXER_G_6DB 0x02
#define UHFMAN_RX_DEMOD_MIXER_G_9DB 0x03
#define UHFMAN_RX_DEMOD_MIXER_G_12DB 0x04
#define UHFMAN_RX_DEMOD_MIXER_G_15DB 0x05
#define UHFMAN_RX_DEMOD_MIXER_G_16DB 0x06

#define UHFMAN_RX_DEMOD_IF_G_12DB 0x00
#define UHFMAN_RX_DEMOD_IF_G_18DB 0x01
#define UHFMAN_RX_DEMOD_IF_G_21DB 0x02
#define UHFMAN_RX_DEMOD_IF_G_24DB 0x03
#define UHFMAN_RX_DEMOD_IF_G_27DB 0x04
#define UHFMAN_RX_DEMOD_IF_G_30DB 0x05
#define UHFMAN_RX_DEMOD_IF_G_36DB 0x06
#define UHFMAN_RX_DEMOD_IF_G_40DB 0x07
 ...
 * @param pMixerG_out Address where the mixer gain value will be stored
 * @param pIfG_out Address where the IF gain value will be stored
 * @param pThrd_out Address where the threshold value will be stored
 */
uhfman_err_t uhfman_get_demod_params(uhfman_ctx_t* pCtx, int* pMixerG_out, int* pIfG_out, uint16_t* pThrd_out);
...
typedef void (*uhfman_poll_handler_t)(uhfman_tag_t tagInfo, uint8_t rssi, void* pUserData);
